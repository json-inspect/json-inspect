<template>
  <v-app>
    <v-app-bar app color="grey darken-3" dark dense extension-height="36">
      ...
      <template v-slot:extension>
        <v-tabs class="ml-3" v-model="tab" show-arrows>
          ...
          <v-btn title="Add tab" small @click="tabAdd" class="mt-1 ml-2" icon>
          ...
          <v-menu open-on-hover="" offset-y>
            <template v-slot:activator="{ on, attrs }">
              <v-btn
                ...
                text
              >
                <v-icon small class="mr-2">mdi-file-compare</v-icon> Compare
              </v-btn>
            </template>
            <v-list>
              <v-list-item
      ...
      <v-btn
        small
        target="_blank"
        href="https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=P57DMM8WRFLWU&item_name=Help+support+the+server/maintenance+costs+and+continued+development+costs+associated+with+jsoninspect.com&currency_code=USD&source=url"
        text
      >
      ...
      <v-btn
        small
        target="_blank"
        href="https://twitter.com/emilyplusplus"
        text
      >
          ...
          <v-row class="ma-4">
            ...
            <v-col cols="12" md="5" class="pb-0">
              <v-card :elevation="darkMode ? 6 : 2" class="pa-2">
                <v-card-actions>
                  <v-icon
                    :title="validJSON().text"
                    small
                    class="ml-1 mr-1"
                    :color="validJSON().color"
                    >mdi-circle</v-icon
                  >
                  <v-spacer></v-spacer>
                  <v-btn
            ...
            <v-col cols="12" md="5" class="pb-0" style="position: relative">
              <v-card
                ...
                class="pa-2 cheightf cscroll"
              >
                <v-card-actions style="overflow: hidden">
                  <v-menu open-on-hover="" offset-y>
                    <template v-slot:activator="{ on, attrs }">
                      <v-btn text small class="" v-bind="attrs" v-on="on">
                  ...
                  <v-layout style="font-size: 0.9em" class="key">
                    <v-spacer></v-spacer>
                    <v-icon small class="ml-2 mr-1" color="#86b25c"
                      >mdi-circle</v-icon
                    >
                    String
                    <v-icon small class="ml-2 mr-1" color="#f9ae58"
                      >mdi-circle</v-icon
                    >
                    Number
                    <v-icon small class="ml-2 mr-1" color="#ec5f66"
                      >mdi-circle</v-icon
                    >
                    Boolean
                  </v-layout>
                ...
                <v-row style="height: 100%" align="center" justify="center">
                  <v-col>
                    <p class="text-center white--text">
                      Your ad could be here, please contact us today!
                    </p>
                  </v-col>
                </v-row>
              </v-card>
    ...
    >
      <v-spacer></v-spacer>
      <div>
        Made with ❤️ by Emily Carlsen
        <v-btn icon target="_blank" href="https://twitter.com/emilyplusplus"
        ...
        <v-btn
          ...
          icon
          target="_blank"
          href="https://www.instagram.com/emilyplusplus/"
          ><v-icon>mdi-instagram</v-icon></v-btn
        >
        <!--<v-btn style="margin-left: -10px;" icon target="_blank" href="https://emilyplusplus.wordpress.com/"><v-icon >mdi-wordpress</v-icon></v-btn>-->
        · Open source software:
        <v-btn icon target="_blank" href="https://github.com/json-inspect"
          ><v-icon>mdi-github</v-icon></v-btn
        >
        · What is JSON? <a target="_blank" href="http://json.org">json.org</a>
      </div>
    </v-footer>
  </v-app>
</template>
...
export default {
  ...
  methods: {
    json2xml(o, tab) {
      ...
      return (
        '<?xml version="1.0" encoding="UTF-8" ?><root>' + final + "</root>"
      );
    },
    ...
    download(i) {
      var dataStr =
        "data:text/json;charset=utf-8," + encodeURIComponent(this.tabs[i].json);
    ...
    downloadXML(i) {
      ...
      let dataStr =
        "data:text/xml;charset=utf-8," + encodeURIComponent(dataRaw);
    ...
    downloadCSV(i) {
      ...
      let dataStr =
        "data:text/csv;charset=utf-8," + encodeURIComponent(dataRaw);
    ...
    beautify(i) {
      this.tabs[i].json = JSON.stringify(
        JSON.parse(this.tabs[i].json),
        null,
        4
      );
    },
    ...
    validJSON: function () {
      ...
      if (this.tabs.length > 0 && this.tab < this.tabs.length)
        source = this.tabs[this.tab].json;
